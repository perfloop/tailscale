...
//
// See tailscale/tailscale#19053, which made REFUSED a soft error.
...
func TestForwarderRcodeWithHungTCP(t *testing.T) {
	...
	// sendTCP bumps this before it dials, so it registers even against an
	// upstream that accepts the connection and never reads it.
	...
	if got := metricDNSFwdTCP.Value() - beforeTCP; got != 0 {
