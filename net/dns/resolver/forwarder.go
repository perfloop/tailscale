...
const (
	...
	// rcodeHoldGrace is how long forwardWithDestChan holds an upstream REFUSED
	// or SERVFAIL after every resolver has started, hoping a healthier one
	// answers. See rcodeHoldDelay.
	rcodeHoldGrace = 300 * time.Millisecond
)
...
func (f *forwarder) send(ctx context.Context, fq *forwardQuery, rr resolverAndDelay) (ret []byte, err error) {
	...
	thenTCP := func(ctx context.Context) ([]byte, error) {
		// A response carrying REFUSED or SERVFAIL is a complete answer, so a
		// TCP retry to the same server can only fetch it again. With TCP/53
		// filtered that costs a SYN retransmit per query.
...
func (f *forwarder) forwardWithDestChan(ctx context.Context, query packet, responseChan chan<- packet, resolvers ...resolverAndDelay) error {
	...
	for {
		...
		case <-holdC:
			// select picks at random when resc is ready too, and a real answer
			// beats a held rcode.
			select {
			case v := <-resc:
				return deliverSuccess(v)
